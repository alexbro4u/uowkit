...
func (e *executor) do(ctx context.Context, fn func(tx Tx) error, attempt int) (retErr error) {
	...
	defer func() {
		...
		e.runAfterRollback(ctx, retErr)
	}()

	if err = fn(tx); err != nil {
		e.log(ctx, slog.LevelDebug, "function returned error, rolling back",
			slog.String("error", err.Error()),
		)
		if rbErr := tx.Rollback(); rbErr != nil {
			e.log(ctx, slog.LevelError, "rollback failed",
				slog.String("error", rbErr.Error()),
			)
			panicked = false
	...
	for _, hook := range e.cfg.beforeCommit {
		if err = hook(ctx, tx); err != nil {
			...
			if rbErr := tx.Rollback(); rbErr != nil {
				panicked = false
				return errors.Join(wrapErr(ErrHookFailed, err), wrapErr(ErrRollbackFailed, rbErr))
			}
			e.runAfterRollback(ctx, err)
			panicked = false
			return wrapErr(ErrHookFailed, err)
		}
	}
